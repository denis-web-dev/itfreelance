document.addEventListener('DOMContentLoaded', function () {
  ...
  // SVG для открытого глаза (без изменений, иконка)
  ...
  // SVG для закрытого глаза (без изменений, исходная иконка)
  ...
  // Инициализация начального состояния иконок (добавлено: установка иконки закрытого глаза при загрузке)
  toggleButtons.forEach((button) => {
    const toggleIcon = button.querySelector('.toggle__password-svg');
    toggleIcon.innerHTML = closedEyeSVG; // Устанавливаем закрытый глаз по умолчанию
    button.setAttribute('aria-label', 'Показать пароль'); // Устанавливаем начальный aria-label
  });
